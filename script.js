...
(function () {
  ...
  function initButtonFire() {
    ...
    const fsrc = [
      ...
      "  float halfW = 0.16 + above*0.22;",
      "  float colmask = smoothstep(halfW+0.2, halfW-0.05, abs(x-0.5));",
      ...
      "  vec3 smoke = mix(vec3(0.55,0.57,0.63), vec3(0.85,0.87,0.93), pow(clamp(ridge,0.0,1.0),4.0));",
      "  gl_FragColor = vec4(smoke, density * 0.6);",
      "}"
    ].join("\n");
